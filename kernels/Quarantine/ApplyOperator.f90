...
CONTAINS
  ...
  SUBROUTINE applyoperator(numDim,dimSizes,numComponents,numPoints,opDir,opInterval,numStencils, &
    ...
    REAL(KIND=8),    INTENT(IN)  :: U(numPoints)
    REAL(KIND=8),    INTENT(OUT) :: dU(numPoints)

    INTEGER(KIND=8) :: I, J, K, jIndex, jkIndex, kIndex
    REAL(KIND=8)    :: fac
    INTEGER(KIND=8) :: plane, pointOffset, iPoint
    INTEGER(KIND=4) :: iStencil, iWeight

    !  WRITE(*,*) 'NUMDIM = ',numDim,'OPDIR = ',opDir
          ...
          DO K = opInterval(5), opInterval(6)
             ...
             DO J = opInterval(3), opInterval(4)
                ...
                DO I = opInterval(1), opInterval(2)
                   iPoint   = jkIndex + I
                   iStencil = stencilID(iPoint)
                   !                 WRITE(*,*) ' PROCESSING POINT ',iPoint 
                   dU(iPoint) = 0.0_8
                   DO  iWeight = stencilStarts(iStencil),stencilStarts(iStencil)+stencilSizes(iStencil) - 1
                      !                    WRITE(*,*) 'DU[',iPoint,'](',I,',',J,',',K,') += ',stencilWeights(iWeight),'*U(',iPoint,&
                      !                         '+',stencilOffsets(iWeight),'*',pointOffset,')'
                      !                    WRITE(*,*) 'DU[',iPoint,'](',I,',',J,',',K,')<',DU(iPoint),'> += ',stencilWeights(iWeight),'*',&
                      !                         U(iPoint+stencilOffsets(iWeight)*pointOffset)
                      dU(iPoint) = dU(iPoint) + stencilWeights(iWeight)*U(iPoint+stencilOffsets(iWeight)*pointOffset)
                   END DO
                END DO
             END DO
          END DO
